<h2>{$translate->_('prefences.account.settings')|capitalize}</h2>
<br>
...
<b>{$translate->_('prefences.account.password.change')|capitalize}</b><br>
<div id="preferences_error" style="color: red; font-weight: bold;"></div>
<table cellspacing="1" cellpadding="0" border="0">
	<tr>
		<td>{$translate->_('prefences.account.password.new')|capitalize}</td>
		<td><input type="password" name="change_pass" value=""></td>
	</tr>
	<tr>
		<td>{$translate->_('prefences.account.password.verify')|capitalize}</td>
		<td><input type="password" name="change_pass_verify"=""></td>
	</tr>
</table>
<br>

<b>{$translate->_('prefences.account.timezone')|capitalize}</b> {if !empty($server_timezone)}({$translate->_('prefences.account.current')} {$server_timezone}){/if}<br>
...
<b>{$translate->_('prefences.account.language')|capitalize}</b> {if !empty($selected_language) && isset($langs.$selected_language)}({$translate->_('prefences.account.current')} {$langs.$selected_language}){/if}<br>
...
<h2>{$translate->_('prefences.account.email')|capitalize}</h2>
{$translate->_('prefences.account.email.associated')}<br>
<br>
<table cellspacing="0" cellpadding="2" border="0">
	<tr>
		<td style="padding-right:10px;"><b>{$translate->_('prefences.account.email.address')|capitalize}</b></td>
		<td style="padding-right:10px;"><b>{$translate->_('prefences.account.email.address.confirmed')|capitalize}</b></td>
		<td><b>Delete</b></td>
	</tr>
	...
	<tr>
		...
		<td style="padding-right:10px;">
			{if $address->is_confirmed}
				yes
			{else}
				no, <a href="javascript:;" onclick="document.resendConfirmationForm.email.value='{$address->address}';document.resendConfirmationForm.submit();">$translate->_('prefences.account.email.address.resend.confirm')}</a>
			{/if}
		</td>
		<td>
...
<b>{$translate->_('prefences.account.assist')|capitalize}</b><br>
...
<label><input type="checkbox" name="mail_inline_comments" value="1" {if $mail_inline_comments}checked{/if}> {$translate->_('prefences.account.mail.comments')}</label><br>
<label><input type="checkbox" name="mail_always_show_all" value="1" {if $mail_always_show_all}checked{/if}> {$translate->_('prefences.account.mail.readall')}</label><br>
<br>
